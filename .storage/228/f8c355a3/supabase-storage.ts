...
export const getRecords = async (): Promise<PrisonRecord[]> => {
  ...
  return data.map(record => ({
    ...
    fixedId: record.fixed_id || '',
    individualPhoto: record.individual_photo || '',
    dateTime: record.date_time,
    location: record.location,
    reason: record.reason,
    articles: Array.isArray(record.articles) ? record.articles : (record.articles ? [record.articles] : []),
...
export const createRecord = async (record: Omit<PrisonRecord, 'id' | 'createdAt'>): Promise<PrisonRecord | null> => {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      fixed_id: record.fixedId,
      individual_photo: record.individualPhoto,
  ...
  return {
    ...
    fixedId: data.fixed_id || '',
    individualPhoto: data.individual_photo || '',
    dateTime: data.date_time,
    location: data.location,
    reason: data.reason,
    articles: Array.isArray(data.articles) ? data.articles : (data.articles ? [data.articles] : []),
...
export const updateRecord = async (recordId: string, updates: Partial<PrisonRecord>): Promise<boolean> => {
  ...
  if (updates.fixedId !== undefined) dbUpdates.fixed_id = updates.fixedId;
  if (updates.individualPhoto !== undefined) dbUpdates.individual_photo = updates.individualPhoto;
  if (updates.location) dbUpdates.location = updates.location;
  if (updates.reason) dbUpdates.reason = updates.reason;
  if (updates.articles !== undefined) dbUpdates.articles = updates.articles;
...
export const getAllIndividuals = async (): Promise<Array<{ name: string; fixedId: string; photo: string; count: number; lastRecord: PrisonRecord }>> => {
  const records = await getRecords();
  const individualsMap = new Map<string, { fixedId: string; photo: string; count: number; lastRecord: PrisonRecord }>();
  ...
  records.forEach(record => {
    ...
    if (existing) {
      ...
      if (new Date(record.dateTime) > new Date(existing.lastRecord.dateTime)) {
        ...
        existing.photo = record.individualPhoto || existing.photo;
      }
    } else {
      individualsMap.set(record.individualName, { 
        fixedId: record.fixedId,
        photo: record.individualPhoto || '',
        count: 1, 
        lastRecord: record 
      });
    }
...
📝 **Registrado por:** {createdBy}`,
    ...
    brasiliaLogoUrl: '/assets/brasilia-logo_variant_1.jpg',
    bopeLogoUrl: '/assets/bope-logo_variant_1.png'
  };
};