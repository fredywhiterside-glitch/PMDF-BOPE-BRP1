...
import { Plus, LogOut, FileText, Edit, Trash2, Search, Home, FolderOpen, Users, UserCheck, BookOpen, UserCircle, Copy } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const handleClone = (record: PrisonRecord) => {
    ...
    localStorage.setItem('clone_record', JSON.stringify({
      individualName: record.individualName,
      screenshots: record.screenshots && record.screenshots.length > 0 ? [record.screenshots[0]] : []
    }));
  ...
  const handleNewRecordForIndividual = (individualName: string, existingPhoto: string) => {
    if (!canCreateRecords()) {
      toast.error('Você não tem permissão para criar registros');
      return;
    }
    ...
    localStorage.setItem('clone_record', JSON.stringify({
      ...
      screenshots: existingPhoto ? [existingPhoto] : []
    }));
                ...
                <CardContent>
                  <ol className="space-y-3 text-slate-300">
                    ...
                    <li className="flex gap-3">
                      ...
                      <span>Ou use o botão "Clonar" em um registro existente para reutilizar a foto</span>
                    </li>
                    <li className="flex gap-3">
                      ...
                      <span>O registro será automaticamente enviado para o Discord com a foto</span>
                    </li>
                  </ol>
                </CardContent>
              </Card>
                      ...
                      {individuals.map((individual) => (
                        ...
                        >
                          <CardContent className="p-6 text-center">
                            <UserCircle className="h-16 w-16 text-yellow-500 mx-auto mb-3" />
                    ...
                    {canCreateRecords() && (
                      <Button
                        onClick={() => {
                          ...
                          const photo = individualRecords[0]?.screenshots?.[0] || '';
                          handleNewRecordForIndividual(selectedIndividual, photo);
                        }}
                        className="bg-gradient-to-r from-yellow-600 to-red-600 hover:from-yellow-700 hover:to-red-700 font-semibold"
                      >
                        <Plus className="h-4 w-4 mr-2" />
