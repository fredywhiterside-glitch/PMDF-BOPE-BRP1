...
import { Plus, LogOut, FileText, Edit, Trash2, Search, Home, FolderOpen, Image as ImageIcon, Users, UserCheck, BookOpen, UserCircle, Copy } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [currentView, setCurrentView] = useState<'home' | 'records' | 'gallery' | 'pending' | 'members' | 'individuals'>('home');
  ...
  const handleClone = (record: PrisonRecord) => {
    if (!canCreateRecords()) {
      toast.error('Você não tem permissão para criar registros');
      return;
    }
    
    // Store the record data to clone in localStorage
    localStorage.setItem('clone_record', JSON.stringify({
      individualName: record.individualName,
      screenshots: record.screenshots
    }));
    
    toast.success('Registro preparado para clonagem!');
    navigate('/new-record');
  };
                ...
                <CardContent>
                  <ol className="space-y-3 text-slate-300">
                    <li className="flex gap-3">
                      ...
                      <span>Clique no botão "Novo Registro" acima ou vá para a aba "Registros"</span>
                    </li>
                    <li className="flex gap-3">
                      ...
                      <span>Preencha todos os campos obrigatórios: nome do indivíduo, localização, motivo e oficiais responsáveis</span>
                    </li>
                    <li className="flex gap-3">
                      ...
                      <span>Adicione fotos de evidência: faça upload ou pressione Ctrl+V para colar</span>
                    </li>
                    <li className="flex gap-3">
                      ...
                      <span>Ou use o botão "Clonar" em um registro existente para reutilizar fotos</span>
                    </li>
                    <li className="flex gap-3">
                      <span className="flex-shrink-0 w-6 h-6 bg-yellow-600 rounded-full flex items-center justify-center text-white text-sm font-bold">5</span>
                      <span>O registro será automaticamente enviado para o Discord com as fotos</span>
                    </li>
                  </ol>
                </CardContent>
              </Card>
            ...
            <div className="space-y-6">
              {!selectedIndividual ? (
                        ...
                        >
                          <CardHeader>
                            ...
                            <CardDescription className="text-slate-400">
                              {individual.count} {individual.count === 1 ? 'prisão registrada' : 'prisões registradas'}
                            </CardDescription>
                          </CardHeader>
                          <CardContent>
                            <div className="space-y-2">
                              <p className="text-slate-400 text-sm">Última prisão:</p>
                              <p className="text-white text-sm">{new Date(individual.lastRecord.dateTime).toLocaleString('pt-BR')}</p>
                              <p className="text-slate-400 text-sm">{individual.lastRecord.location}</p>
                              {individual.lastRecord.screenshots && individual.lastRecord.screenshots.length > 0 && (
                                <img
                                  src={individual.lastRecord.screenshots[0]}
                                  alt={individual.name}
                ...
                <>
                    ...
                    {canCreateRecords() && (
                      <Button
                        onClick={() => {
                          ...
                          const photos = individualRecords[0]?.screenshots || [];
                          handleNewRecordForIndividual(selectedIndividual, photos);
                        }}
                        className="bg-gradient-to-r from-yellow-600 to-red-600 hover:from-yellow-700 hover:to-red-700 font-semibold"
                      >
                        <Plus className="h-4 w-4 mr-2" />
                  ...
                  <div className="grid gap-4">
                    {getRecordsByIndividual(selectedIndividual).map((record) => (
                      <Card key={record.id} className="bg-slate-900/80 border-slate-700 hover:bg-slate-900 transition-all backdrop-blur-xl">
                        <CardHeader>
                          <div className="flex justify-between items-start">
                            <div>
                              <CardTitle className="text-white text-xl">Prisão #{getRecordsByIndividual(selectedIndividual).indexOf(record) + 1}</CardTitle>
                              <CardDescription className="text-slate-400 mt-1">
                                {new Date(record.dateTime).toLocaleString('pt-BR')} • {record.location}
                              </CardDescription>
                            </div>
