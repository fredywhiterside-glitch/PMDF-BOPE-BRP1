...
export default function NewRecord() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save record locally first - THIS ALWAYS SUCCEEDS
      saveRecord(record);
      ...
      // Try to send to Discord in background (don't wait for it)
      setTimeout(() => {
        sendToDiscord(record)
          .then((success) => {
            if (success) {
              console.log('Registro enviado para Discord com sucesso');
            } else {
              console.warn('Erro ao enviar para Discord');
            }
          })
          .catch((error) => {
