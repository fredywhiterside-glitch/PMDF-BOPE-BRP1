...
const sendToDiscord = async (
  ...
  try {
    ...
    // Format articles list - extract only "SU XXX" format
    const articlesList = record.articles
      .map(article => {
    ...
    // Create the message content WITHOUT the header line
    const content = `**Oficial Responsável:** ${record.responsibleOfficers}
...
**Artigos Aplicados:** ${articlesList}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((sendToDiscordEnabled && discordSuccess) || (saveToDatabase && databaseSuccess)) {
        toast.success('Ficha criminal criada com sucesso!');
      ...
      toast.error('Erro ao criar ficha criminal. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 relative overflow-hidden">
      ...
      <div 
        ...
        style={{ 
          ...
          filter: 'brightness(0.4) sepia(1) hue-rotate(200deg) saturate(3)'
        }}
      />
          ...
          <CardHeader>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <CardTitle className="text-white text-2xl">Nova Ficha Criminal</CardTitle>
