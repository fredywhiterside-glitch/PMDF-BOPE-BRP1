...
export const sendToDiscord = async (record: PrisonRecord): Promise<boolean> => {
  ...
  try {
    // Send message with embeds for images
    const payload: any = {
      content: message,
      embeds: []
    };

    // Add images as embeds (Discord supports up to 10 embeds)
    if (record.screenshots && record.screenshots.length > 0) {
      record.screenshots.slice(0, 10).forEach((screenshot, index) => {
        payload.embeds.push({
          ...
          color: 0xff0000,
          footer: index === record.screenshots.length - 1 ? {
...
export const getAllIndividuals = (): Array<{ name: string; count: number; lastRecord: PrisonRecord }> => {
  ...
  individualsMap.forEach((recordsList, name) => {
    ...
    individuals.push({
      name: recordsList[0].individualName, // Use original case
