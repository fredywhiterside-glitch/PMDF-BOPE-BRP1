...
import { saveRecord, sendToDiscord, getSettings, addLog, getRecords } from '@/lib/storage';
...
export default function NewRecord() {
    ...
    if (cloneData) {
      try {
        ...
        if (parsed.screenshots && Array.isArray(parsed.screenshots)) {
          // Filter out invalid images (non-base64)
          const validImages = parsed.screenshots.filter((img: string) => {
            const isValid = isValidBase64Image(img);
            if (!isValid) {
              console.warn('⚠️ Imagem inválida ignorada:', img.substring(0, 50));
            }
            return isValid;
          });
          ...
          if (validImages.length > 0) {
            ...
            toast.success(`Dados carregados! ${validImages.length} foto(s) válida(s).`);
          } else {
            toast.warning('Dados carregados, mas nenhuma foto válida encontrada. Adicione novas fotos.');
          }
        }
        ...
        localStorage.removeItem('clone_record');
        toast.error('Erro ao carregar dados clonados');
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('🔵 [DEBUG] Iniciando salvamento do registro:', record.id);
      console.log('🔵 [DEBUG] Nome:', record.individualName);
      console.log('🔵 [DEBUG] Fotos válidas:', record.screenshots.length);
      console.log('🔵 [DEBUG] Primeira foto (primeiros 50 chars):', record.screenshots[0]?.substring(0, 50));

      // Step 1: Send to Discord FIRST (with images)
      toast.info('Enviando para Discord...');
      const discordSuccess = await sendToDiscord(record);
      
      if (!discordSuccess) {
        console.warn('⚠️ [DEBUG] Falha ao enviar para Discord');
        toast.warning('Erro ao enviar para Discord, mas o registro será salvo localmente');
      } else {
        console.log('✅ [DEBUG] Enviado para Discord com sucesso');
        toast.success('Enviado para Discord com sucesso!');
      }

      // Step 2: Save to local database AFTER Discord
      console.log('🔵 [DEBUG] Salvando no localStorage...');
      console.log('🔵 [DEBUG] Registros antes do save:', getRecords().length);
      
      saveRecord(record);
      
      // Verify save was successful
      const recordsAfterSave = getRecords();
      console.log('🔵 [DEBUG] Registros após save:', recordsAfterSave.length);
      
      const savedRecord = recordsAfterSave.find(r => r.id === record.id);
      if (savedRecord) {
        console.log('✅ [DEBUG] Registro salvo com sucesso no localStorage!');
        console.log('✅ [DEBUG] Registro encontrado:', savedRecord.individualName);
        console.log('✅ [DEBUG] Fotos salvas:', savedRecord.screenshots.length);
      } else {
        console.error('❌ [DEBUG] ERRO: Registro NÃO foi salvo no localStorage!');
        throw new Error('Falha ao salvar registro no localStorage');
      }
      
      // Step 3: Add log entry
      console.log('🔵 [DEBUG] Adicionando log de auditoria...');
      addLog({
        action: 'create',
        performedBy: user.username,
        details: `Criou um novo registro de prisão: ${record.individualName}`
      });
      console.log('✅ [DEBUG] Log adicionado');
      ...
      console.error('❌ [DEBUG] Erro ao criar registro:', error);
