...
export const sendToDiscord = async (record: PrisonRecord): Promise<boolean> => {
  ...
  try {
    ...
    const textResponse = await fetch(settings.webhookUrl, {
      method: 'POST',
      headers: {
    ...
    // Try to upload and send images if they exist
    if (record.screenshots && record.screenshots.length > 0) {
      try {
        // Wait a bit to avoid rate limiting
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        // Upload images to ImgBB (with timeout)
        const uploadPromise = uploadMultipleImages(record.screenshots);
        ...
        console.error('Error uploading images, but text was sent:', imageError);
