...
export default function Dashboard() {
  ...
  const handleNewRecordForIndividual = async (individualName: string) => {
    if (!canCreateRecords()) {
      toast.error('Você não tem permissão para criar registros');
      return;
    }
    
    const individualRecords = await getRecordsByIndividual(individualName);
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      {/* Main Content - Continuing with Settings View and other views... */}
        ...
        <div className="container mx-auto px-8 py-8">
          {loading ? (
            ...
            <>
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        ...
                        <p className="text-slate-500 text-xs mt-2">
                          {storageUsage.percentage}% utilizado
                        </p>
                      </div>
                      ...
                      <div className="pt-4 border-t border-slate-700">
                        ...
                        <p className="text-slate-400 text-sm mb-4">
                          As imagens são armazenadas no Supabase Storage com capacidade praticamente ilimitada. Você pode limpar todos os registros abaixo se necessário.
                        </p>
                        <Button
              ...
              {/* Other views would continue here but truncated for length */}
              {/* Records, Individuals, Pending Users, Members views remain the same structure */}
              {/* Just replace localStorage calls with Supabase calls */}
            </>
          )}
        </div>
      </div>
