...
import { createRecord, createLog, getRecordsByIndividual, uploadImage, getStorageUsage } from '@/lib/supabase-storage';
      ...
      if (individualData) {
        try {
          ...
          if (parsed.individualName) {
            ...
            // Load existing photos for this individual
            const records = await getRecordsByIndividual(parsed.individualName);
            const photos: string[] = [];
            records.forEach(record => {
              if (record.screenshots && record.screenshots.length > 0) {
                photos.push(...record.screenshots);
              }
            });
            
            if (photos.length > 0) {
              setAvailablePhotos(photos);
              toast.success(`${photos.length} foto(s) disponível(is) para seleção`);
            }
          }
    ...
    try {
      ...
      if (saveToDatabase) {
        ...
        for (let i = 0; i < screenshots.length; i++) {
          ...
          // Check if it's already a Supabase URL (from existing photos)
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  required
                  className="bg-slate-800 border-slate-600 text-white focus:border-yellow-500"
                  placeholder="ID do servidor do indivíduo"
                  disabled={loading}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent className="bg-slate-800 border-slate-600">
                    <SelectItem value="Tráfico de Drogas" className="text-white hover:bg-slate-700">
                      Tráfico de Drogas
                    </SelectItem>
                    <SelectItem value="Porte Ilegal de Armas" className="text-white hover:bg-slate-700">
                      Porte Ilegal de Armas
                    </SelectItem>
                    <SelectItem value="Roubo/Furto" className="text-white hover:bg-slate-700">
                      Roubo/Furto
                    </SelectItem>
                    <SelectItem value="Homicídio" className="text-white hover:bg-slate-700">
                      Homicídio
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                <Label htmlFor="penalty" className="text-white">
                  Pena Aplicada *
                </Label>
                <Input
                  id="penalty"
                  ...
                  required
                  className="bg-slate-800 border-slate-600 text-white focus:border-yellow-500"
                  placeholder="Ex: 10 anos de prisão"
                  disabled={loading}
                />
              </div>
