...
import { ArrowLeft, Save, Loader2, Image as ImageIcon, X, Upload, Check } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (saveToDatabase) {
        ...
        console.log('🔵 [DATABASE] localStorage antes:', localStorage.getItem('prison_records')?.substring(0, 100));
        
        const recordsBeforeSave = getRecords();
        console.log('🔵 [DATABASE] Registros ANTES do save:', recordsBeforeSave.length);
        console.log('🔵 [DATABASE] IDs existentes:', recordsBeforeSave.map(r => r.id));
        
        toast.info('Salvando no banco de dados...');
        
        // Direct save to localStorage with verification
        try {
          const currentRecords = getRecords();
          const newRecords = [...currentRecords, record];
          localStorage.setItem('prison_records', JSON.stringify(newRecords));
          console.log('🔵 [DATABASE] localStorage.setItem executado');
          console.log('🔵 [DATABASE] localStorage após:', localStorage.getItem('prison_records')?.substring(0, 100));
          ...
          console.log('🔵 [DATABASE] IDs após save:', recordsAfterSave.map(r => r.id));
          
          const savedRecord = recordsAfterSave.find(r => r.id === record.id);
          
          if (savedRecord) {
            console.log('✅ [DATABASE] SUCESSO! Registro encontrado no banco');
            console.log('✅ [DATABASE] Nome salvo:', savedRecord.individualName);
            console.log('✅ [DATABASE] Fotos salvas:', savedRecord.screenshots.length);
            databaseSuccess = true;
            toast.success('✅ Salvo no banco de dados!');
          } else {
            console.error('❌ [DATABASE] ERRO CRÍTICO: Registro NÃO encontrado após save!');
            console.error('❌ [DATABASE] Esperado ID:', record.id);
            console.error('❌ [DATABASE] IDs encontrados:', recordsAfterSave.map(r => r.id));
            throw new Error('Registro não foi salvo no banco de dados');
          }
        } catch (saveError) {
          console.error('❌ [DATABASE] Erro durante salvamento:', saveError);
          throw saveError;
        }
        
        // Add log entry
        console.log('🔵 [LOG] Adicionando log de auditoria...');
        addLog({
          action: 'create',
          performedBy: user.username,
          details: `Criou um novo registro de prisão: ${record.individualName}`
        });
        console.log('✅ [LOG] Log adicionado');
      } else {
      ...
      // Final status
      console.log('🔵 [FINAL] ========================================');
      console.log('🔵 [FINAL] Discord:', discordSuccess ? '✅ Sucesso' : sendToDiscordEnabled ? '❌ Falhou' : '⏭️ Desabilitado');
      console.log('🔵 [FINAL] Database:', databaseSuccess ? '✅ Sucesso' : saveToDatabase ? '❌ Falhou' : '⏭️ Desabilitado');
      console.log('🔵 [FINAL] ========================================');

      if ((sendToDiscordEnabled && discordSuccess) || (saveToDatabase && databaseSuccess)) {
        toast.success('Registro criado com sucesso!');
        console.log('✅ [FINAL] Navegando para dashboard...');
        
        // Delay before navigation to ensure everything is saved
