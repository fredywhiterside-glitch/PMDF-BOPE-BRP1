...
import { saveRecord, sendToDiscord, getSettings } from '@/lib/storage';
...
export default function NewRecord() {
  ...
  useEffect(() => {
    ...
    // Load pre-selected images from gallery
    const selectedImages = localStorage.getItem('selected_images');
    if (selectedImages) {
      const images = JSON.parse(selectedImages);
      setScreenshots(images);
      localStorage.removeItem('selected_images');
      toast.success(`${images.length} ${images.length === 1 ? 'foto carregada' : 'fotos carregadas'} da galeria`);
    }
                  ...
                  <div className="text-center mb-3">
                    ...
                    <div className="flex gap-3 justify-center flex-wrap">
                      ...
                      <div className="text-slate-400 text-sm flex items-center">
                        ou <kbd className="px-2 py-1 bg-slate-700 rounded mx-1">Ctrl+V</kbd>
                      </div>
                    </div>
                    <input
                      ref={fileInputRef}
